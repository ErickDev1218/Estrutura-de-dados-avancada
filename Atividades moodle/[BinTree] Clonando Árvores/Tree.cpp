...
bool Tree::_identical(Node *nd1, Node *nd2) { // TODO
    if(nd1 == nullptr && nd2 == nullptr){ //são iguais
        return true;
    }else if(nd1 != nullptr && nd2 == nullptr || nd2 != nullptr && nd1 == nullptr){ //são diferentes
        return false;
    }else if(nd1->key != nd2->key){// sao diferentes
        return false;
    }
...
Node *Tree::_clone(Node *node) { // TODO
    ...
    Node *n = new Node(node->key,_clone(node->left),_clone(node->right));
    return n;
}
