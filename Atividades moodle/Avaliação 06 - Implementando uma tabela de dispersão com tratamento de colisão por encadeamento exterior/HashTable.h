...
size_t HashTable<K,V>::count(const K& k) const 
{
    // Fazer
    size_t final = 0;
    size_t bucket = hash_code(k); //Pegando o local onde esse valor pode estar
    //Preciso percorrer essa lista para saber se K está ou não.
    for(auto i = (*_table)[bucket].begin(); i != (*_table)[bucket].end(); ++i){
        if((*i).first == k){
            final +=1;
        }
    }
...
{
    // Fazer
    //Retorno a quantidade de slots o vector tem.
    //Acredito que (*table).size() também resolveria.
    return _table_size;
}
...
{
    // Fazer
    //Testo se n > bucket_size, pois não há como acessar este local
    if(n > bucket_count()){
        throw std::out_of_range("nao ha este bucket");
    }else{
        return (*_table)[n].size();
    }
}

// Returns the bucket number where the element 
//with key k is located or should be contained.
template <typename K, typename V>
...
HashTable<K,V>::HashTable(const HashTable& t) 
{
    ...
    _table = new std::vector<std::list<std::pair<K,V>>>;
    this->_n = t._n;
    this->_table_size = t._table_size;
    this->_load_factor = t._load_factor;
    this->_max_load_factor = t._max_load_factor;
    this->_hashing = t._hashing;
    
    _table->resize(_table_size); //Faço meu vector ter o tamanho que deve ter
    std::vector<std::list<std::pair<K, V>>> *copy = t._table;
    for(int i = 0; i < _table_size; i++){
        //O primeiro for desce no vector.
        if((*copy)[i].size() > 0){
            for(auto &par :(*copy)[i]){
                (*_table)[i].push_back(std::make_pair(par.first,par.second));
            }
        }
    }
}
